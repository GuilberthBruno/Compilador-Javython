...
class SemanticAnalyzer:
    ...
    def analyze_declarations(self, declarations):
        ...
        if not declarations or declarations[0] != 'declarations':
            return
        ...
        if declarations and declarations[0] == 'declarations':
            for decl in declarations[1]:
                if decl[0] == 'declare_group':
                    ...
                    # Analisa o valor da constante primeiro
                    value_type = self.analyze_expression(value)

                    # Se a variável já existe, verifica se é uma atribuição válida
                    if self.is_symbol_defined(var_name):
                        existing_symbol = self.get_symbol(var_name)
                        expected_type = existing_symbol['type']

                        if value_type and expected_type != value_type:
                            self.add_error(
                                f"Tipo incompatível na atribuição: '{var_name}' é do tipo '{expected_type}' mas recebeu '{value_type}'")

                        # Verifica se é constante
                        if existing_symbol.get('is_const', False):
                            self.add_error(f"Não é possível modificar a constante '{var_name}'")
                    else:
                        # Nova constante - infere tipo do valor
                        var_type = self.infer_type_from_value(value)
    ...
    def analyze_statement(self, stmt):
        ...
        if stmt_type == 'assign':
            ...
            if not self.is_symbol_defined(var_name):
                ...
                if expr_type and var_type != expr_type:
                    self.add_error(
                        f"Tipo incompatível na atribuição: '{var_name}' é do tipo '{var_type}' mas recebeu '{expr_type}'")

        elif stmt_type == 'const_assign':
            var_name = stmt[1]
            expr = stmt[2]
            ...
            if self.is_symbol_defined(var_name):
                ...
                if expr_type and expected_type != expr_type:
                    self.add_error(
                        f"Tipo incompatível na atribuição: '{var_name}' é do tipo '{expected_type}' mas recebeu '{expr_type}'")
        ...
        elif stmt_type == 'if' or stmt_type == 'if-else':
            condition = stmt[1]
            if_block = stmt[2]

            self.analyze_condition(condition)
            self.analyze_statement(if_block)
    ...
    def analyze_function_call(self, call_stmt):
        """Analisa uma chamada de função"""
        func_name = call_stmt[1]
        args = call_stmt[2] if len(call_stmt) > 2 else []
        ...
        if len(args) != len(expected_params):
            self.add_error(
                f"Número incorreto de argumentos para '{func_name}'. Esperado: {len(expected_params)}, Obtido: {len(args)}")
            return None
        ...
        for i, (arg, param) in enumerate(zip(args, expected_params)):
            ...
            if arg_type != param_type:
                self.add_error(
                    f"Tipo incorreto para argumento {i + 1} de '{func_name}'. Esperado: '{param_type}', Obtido: '{arg_type}'")
    ...
    def analyze_condition(self, condition):
        ...
        if condition[0] == 'condition':
            ...
            # Condição simples (expressão booleana)
            expr_type = self.analyze_expression(condition)
            if expr_type and expr_type != 'bool':
                self.add_error(f"Condição deve ser do tipo bool, mas recebeu '{expr_type}'")
    ...
    def analyze_expression(self, expr):
        ...
        if isinstance(expr, tuple):
            if expr[0] == 'binop':
                ...
                elif op == '+' and left_type == 'str' and right_type == 'str':
                    return 'str'
    ...
    def infer_type_from_value(self, value):
        ...
        if isinstance(value, tuple):
            if value[0] == 'call':
                ...
                # Para outras expressões, analisa normalmente
                return self.analyze_expression(value)
            ...
            if value.startswith('"') and value.endswith('"'):
                ...
                # É uma variável
                if self.is_symbol_defined(value):
                    return self.get_symbol(value)['type']
                return 'unknown'
